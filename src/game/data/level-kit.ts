...
export type KitPieceKind = 'ground' | 'ledge' | 'slope' | 'breakable' | 'secret-wall' | 'backdrop';
...
export function expandKitCollision(
  ...
  for (const piece of pieces) {
    if (piece.kind === 'backdrop' || piece.kind === 'secret-wall') {
      continue;
    }
    ...
    if (piece.kind === 'slope') {
      rects.push(...expandSlope(piece));
      continue;
    }
    rects.push({ x: piece.x, y: piece.y, w: piece.w, h: piece.h });
  }
  return rects;
}
